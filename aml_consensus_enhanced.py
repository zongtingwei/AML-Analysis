...
# -*- coding: utf-8 -*-
"""
AML enhanced consensus clustering (paper-style) with:
 - 20 combinations = 5 feature selectors × 4 clustering algorithms
 - PAC curve across K (Şenbabaoğlu et al., 2014) to pick K*
 - Final consensus-based labels (Agglomerative on 1 - C)
 - Sankey (vertical) vs WHO Fusion-17 (from Dx198.xlsx)
 - Optional scRNA differentiation scores (HSPC/GMP/Monocyte) via simple z-score mean

Outputs (under --outdir, prefix Dx198_CONSENSUS_k{K*} unless overridden):
  1) *_groups.csv                # Sample -> G1..Gn
  2) *_group_fusion_counts.csv   # long table for alluvial
  3) *_group_fusion_table.csv    # wide cross-tab G × Fusion17
  4) *_group_summary.csv         # size/top_fusion/purity/is_novel
  5) *_novel_members.csv         # all novel groups members (if any)
  6) *_samples_G*.csv            # per-novel-group sample lists
  7) *_consensus_matrix.csv      # Cbar at K*
  8) *_pac_curve.png/pdf         # PAC vs K
  9) *_consensus_heatmap.pdf     # heatmap of Cbar
 10) *_signature_scores.csv      # optional HSPC/GMP/Mono scores per sample

CLI example:
python aml_consensus_enhanced.py \
  --expr AML.by_samle_celltype_mean_exp.csv \
  --dx Dx198.xlsx \
  --outdir out_consensus \
  --kmin 3 --kmax 12 \
  --repeats 80 \
...
def select_dx198(X_full: pd.DataFrame, dx_path: Path) -> Tuple[pd.DataFrame, pd.Series]:
    dx = pd.read_excel(dx_path)
    need = ['Biaoben','Fusion gene subtyping']
    for col in need:
        if col not in dx.columns:
            raise ValueError(f"Dx198.xlsx 需要包含列: {need}, 缺少 {col}")
...
def cluster_agg_cosine(Xp: np.ndarray, k: int) -> np.ndarray:
    # cosine distance = 1 - cosine_similarity; we compute directly from normalized rows
    Xn = normalize(Xp, norm='l2')
    D = 1 - (Xn @ Xn.T)
    D[D<0] = 0.0
...
def consensus_one_combo(X: pd.DataFrame, k: int, repeats: int, subsample_samples: float, subsample_genes: float,
    ...
    """
    Return (C, P): co-occurrence and pair-count matrices (n×n) for this combo at given k.
    """
...
def pac_score(C: np.ndarray, P: np.ndarray, lower: float=0.1, upper: float=0.9) -> float:
    """PAC = fraction of consensus entries in (lower, upper). Lower is better."""
...
def build_counts(groups: pd.Series, fusion: pd.Series) -> pd.DataFrame:
...
def plot_consensus_heatmap(Cbar, labels, out_pdf, title="Consensus matrix (K*)"):
    # order by hierarchical clustering on 1-Cbar
    D = 1 - Cbar
...
def load_signatures(path: Optional[Path]) -> Dict[str, Set[str]]:
    """
    Expect a TSV/CSV with columns: set,gene  (case-insensitive).
    """
    if path is None:
        return {}
...
def run_pipeline(expr: Path, dx: Path, outdir: Path,
    ...
    # choose K*: lowest mean PAC (break ties by smallest K)
    best_idx = int(np.argmin(pac_mean))
    Kstar = ks[best_idx]
    ...
    # export consensus, PAC plots
    pref = f"Dx198_CONSENSUS_k{Kstar}"
...
# -------------------- CLI --------------------
def main():
    ap = argparse.ArgumentParser(description="AML paper-style enhanced consensus clustering (20 combos) + PAC + Sankey + optional scRNA signatures")
